...
void main() {
  ...
  group('JpVocabService Tests', () {
    ...
    late JpVocabService service;

    setUp(() async {
      tempDir = await Directory.systemTemp.createTemp('jp_vocab_');
    ...
    test('JpVocabService lookup calls local LLM generate and returns vocabulary analysis text', () async {
      ollama.reply = '1. 詞性\n  - 名詞\n2. 中文意思';

      final result = await service.lookup('勉強');

      expect(result, '1. 詞性\n  - 名詞\n2. 中文意思');
      expect(ollama.lastPrompt, contains('請提供以下日文單字的詳細資訊：'));
      expect(ollama.lastPrompt, contains('勉強'));
    });
  });
}
