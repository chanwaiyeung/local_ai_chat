...
void main() {
  ...
  group('JpGrammarService Tests', () {
    ...
    late JpGrammarService service;

    setUp(() async {
      tempDir = await Directory.systemTemp.createTemp('jp_grammar_');
      ...
      service = JpGrammarService(router);
    });
    ...
    test('JpGrammarService analyze calls local LLM generate and returns grammar analysis text', () async {
      ollama.reply = '1. 文法結構\n  - Concept 1\n2. 詞性分解';

      final result = await service.analyze('日本語の勉強はとても面白いです。');

      expect(result, '1. 文法結構\n  - Concept 1\n2. 詞性分解');
      expect(ollama.lastPrompt, contains('請解析以下日文句子，格式如下：'));
      expect(ollama.lastPrompt, contains('日本語の勉強はとても面白いです。'));
    });
  });
}
