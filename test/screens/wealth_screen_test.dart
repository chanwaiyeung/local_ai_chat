...
void main() {
  ...
  Future<void> switchToAllocation(WidgetTester tester) async {
    await tester.tapAt(const Offset(750, 95));
    await settle(tester);
  }
  ...
  group('shell + tabs', () {
    ...
    testWidgets('shows empty state on 紀錄 tab when no records', (tester) async {
      await pumpHost(tester);
      expect(find.text('尚無投資紀錄'), findsAtLeastNWidgets(1));
    });

    testWidgets('shows empty state on 配置 tab when no records', (tester) async {
      await pumpHost(tester);
      await switchToAllocation(tester);
      await settle(tester);
  ...
  group('records tab', () {
    ...
    testWidgets('currency picker appears when ≥2 currencies present',
      ...
      await settle(tester);
      expect(find.text('幣別：'), findsAtLeastNWidgets(1));
  ...
  group('form dialog', () {
    testWidgets('FAB opens form with all expected fields', (tester) async {
      ...
      expect(find.textContaining('標籤'), findsAtLeastNWidgets(1));
    });
    ...
    testWidgets('amount validator rejects empty / non-numeric / zero',
      ...
      // Submit zero
      await tester.enterText(find.widgetWithText(TextFormField, '金額'), '0');
      await tapSave(tester);
      expect(find.text('請輸入有效金額'), findsOneWidget);
    });
    ...
    testWidgets('valid submit creates a record', (tester) async {
      ...
      await tester.enterText(find.widgetWithText(TextFormField, '金額'), '12345');
      await tapSave(tester);
