...
class _WealthScreenState extends State<WealthScreen>
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('投資理財'),
        bottom: TabBar(
          controller: _tabCtrl,
          tabs: const [
            Tab(text: '紀錄', icon: Icon(Icons.list)),
            Tab(text: '配置', icon: Icon(Icons.pie_chart_outline)),
          ],
        ),
      ),
      body: TabBarView(
      ...
      children: [
        ...
        Expanded(
          child: records.isEmpty
              ? const Center(child: Text('尚無投資紀錄'))
              : ListView.separated(
  ...
  Widget build(BuildContext context) {
    if (stats.isEmpty) {
      return const Card(
        ...
        child: Padding(padding: EdgeInsets.all(16), child: Text('尚無投資紀錄')),
      );
    }
    return Card(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              ...
              Text('淨值總覽（${stats.currency}）',
                  style: Theme.of(context).textTheme.titleMedium),
              const Spacer(),
              Text('${stats.assetCount} 項資產',
                  style: TextStyle(color: Theme.of(context).hintColor)),
            ]),
            const Divider(),
...
class _CurrencyPicker extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      Text('幣別：', style: TextStyle(color: Theme.of(context).hintColor)),
      const SizedBox(width: 8),
      Expanded(
        child: Wrap(
          spacing: 6,
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              Text('淨值趨勢', style: Theme.of(context).textTheme.titleMedium),
              const Spacer(),
              Text(currency,
                  style: TextStyle(color: Theme.of(context).hintColor)),
            ]),
            ...
            if (history.length < 2)
              const Padding(
                padding: EdgeInsets.symmetric(vertical: 24),
                child: Center(child: Text('需要至少兩個不同日期才能繪製趨勢')),
              )
            else
              SizedBox(
                height: 140,
    ...
    return SingleChildScrollView(
      ...
      child: Form(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            Text(widget.existing == null ? '新增投資紀錄' : '編輯投資紀錄',
            ...
            DropdownButtonFormField<String>(
              // Required for controlled dropdown state in this form.
              // ignore: deprecated_member_use
              value: _assetType,
              decoration: const InputDecoration(labelText: '資產類別'),
              items: [
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                labelText: '資產名稱（選填，例如 AAPL / 0050.TW）',
              ),
            ),
            const SizedBox(height: 8),
            Row(children: [
