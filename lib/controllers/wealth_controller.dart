...
class WealthController extends ChangeNotifier {
  ...
  Map<String, double> getAllocationByType({required String currency}) {
    ...
    for (final r in latestPerAsset().values) {
      ...
      out.update(r.assetType, (v) => v + r.amount,
          ifAbsent: () => r.amount);
    }
    return out;
  }
  ...
  Future<WealthRecord> saveRecord(WealthRecord record) async {
    ...
    final finalRecord = isNew
        ? record.copyWith(id: _generateId())
        : record;
  ...
  Map<String, dynamic> getMonthlyReport({
    ...
    for (final r in inCurrency) {
      if (r.date.isAfter(thisMonthStart.subtract(const Duration(days: 1))) &&
          r.date.isBefore(thisMonthEnd.add(const Duration(days: 1)))) {
        thisMonthTotal += r.amount;
      }
      if (r.date.isAfter(lastMonthStart.subtract(const Duration(days: 1))) &&
          r.date.isBefore(lastMonthEnd.add(const Duration(days: 1)))) {
  ...
  /// Export all records as CSV string (ready for clipboard)
  String exportToCsv() {
