...
import 'long_context_optimizer.dart';
...
class RagService {
  ...
  LongContextTrace? lastLongContextTrace;

  RagService({required this.embedder, required this.store});

  Future<int> indexFile(String path) async {
    final text = await loadDocument(path);
    final docName = path.split(RegExp(r'[\\/]')).last;
    return ingest(docName: docName, text: text);
  }
  ...
  Future<List<ScoredChunk>> retrieve(
    ...
    if ((enableLongContext || enableDeepLongContext) &&
      ...
      lastLongContextTrace = result.trace;
